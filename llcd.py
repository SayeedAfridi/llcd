...
if __name__ == '__main__':
    ...
    for course in config.COURSES:
        ...
        for chapter in chapters:
            no_digit = re.sub(r'^\d+\.', "", chapter['title'])
            no_space = no_digit.lstrip()
            chapter_name = re.sub(r'[\\/*?:"<>|]', "", no_space)
            ...
            for video in videos:
                ...
                try:
                    ...
                    video_file_path = Path('out/%s/0%s - %s' % (course_name, str(chapter_count), chapter_name), '0%s - %s.mp4' % (str(vc), video_name))
                    #video_exists = os.path.exists(video_file_path)
                    if not video_file_path.is_file():
                        download_file(download_url, 'out/%s/0%s - %s' % (course_name, str(chapter_count), chapter_name), '0%s - %s.mp4' % (str(vc), video_name))
                    else:
                        print '[*] ------ Already Downloaded "%s"' % video_name
